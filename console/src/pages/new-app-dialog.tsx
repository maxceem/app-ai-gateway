  ...
  const ready =
    ...
    (applicationType === "server" ||
      (presetsComplete &&
        ...
        // Both scope the app to one tenant, and the Worker refuses a write
        // without them, so the dialog cannot offer to create one either.
        authConfig.issuer.length > 0 &&
        authConfig.audience.length > 0 &&
    ...
    try {
      const result = await createApp.mutateAsync({
        ...
        config: {
           ...
           *
           * A server app gets no default at all. Its callers are one trusted
           * backend, and unless it sends `x-end-user-id` every request shares
           * one identity — the API key's own id — so a per-user limit would
           * not meter users, it would cap the whole backend.
           */
          ...(applicationType === "server" ? {} : {
            limits: {
              per_user: {
