...
import apple from "@/assets/brands/apple.svg?raw";
import auth0 from "@/assets/brands/auth0.svg?raw";
...
import clerk from "@/assets/brands/clerk.svg?raw";
import deepseek from "@/assets/brands/deepseek.svg?raw";
import firebase from "@/assets/brands/firebase.svg?raw";
...
import revenuecat from "@/assets/brands/revenuecat.svg?raw";
import supabase from "@/assets/brands/supabase.svg?raw";
...
/**
 * The vendors an application's authentication policy names: the identity
 * providers the issuer presets configure, and the entitlement sources the
 * entitlement presets do. Keyed by the preset id in `lib/presets.ts`, with two
 * deliberate differences from that list — the `custom` presets name no vendor,
 * so they have no mark, and `apple` has no preset at all, because Sign in with
 * Apple is configured by hand from the `iss` and `aud` the docs give.
 */
export type AuthBrand = "firebase" | "supabase" | "auth0" | "clerk" | "apple" | "revenuecat";

const AUTH_MARKS: Record<AuthBrand, string> = marks({
  firebase,
  supabase,
  auth0,
  clerk,
...
/** Whether a preset id is also a brand with a mark: `custom` and `none` are not. */
export function isAuthBrand(value: string): value is AuthBrand {
  return Object.hasOwn(AUTH_MARKS, value);
}
...
export function AuthBrandIcon({ type, className }: { type: AuthBrand; className?: string }) {
  return <Mark svg={AUTH_MARKS[type]} className={className} />;
}
