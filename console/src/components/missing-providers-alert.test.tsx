...
describe("unconfiguredProviders", () => {
  ...
  /*
   * An app that allows every provider has picked none of them, so there is
   * nothing it is missing: the organization's key list is what it reaches, and
   * naming every provider it has not bought would warn about a choice nobody
   * made. Only a policy that names providers can name one that is unconfigured.
   */
  it("reports nothing for an app that allows every provider", () => {
    expect(unconfiguredProviders(ALL, [credential({})])).toEqual([]);
    expect(unconfiguredProviders(ALL, [])).toEqual([]);
  });
});
...
describe("MissingProvidersAlert", () => {
  ...
  it("says nothing for an app that allows every provider, whatever the org has", async () => {
    stubApi({ "/v1/admin/providers": { body: { providers: [] } } });
    const { client } = renderAuthenticated(<MissingProvidersAlert proxy={ALL} />);
