...
export function PresetPreview({
  ...
  fragment: { jwks_url?: string; issuer?: string; audience?: string; claims: string[] };
}) {
  const settings = (["jwks_url", "issuer", "audience"] as const)
    .map((field) => [field, fragment[field]] as const)
    .filter((entry): entry is readonly [(typeof entry)[0], string] => Boolean(entry[1]));
  if (settings.length === 0 && fragment.claims.length === 0) return null;
  return (
    <div className="space-y-1.5 rounded-md border bg-muted/40 p-3">
      <p className="text-xs font-medium">This writes</p>
      {settings.map(([field, value]) => (
        <p key={field} className="font-mono text-[11px] break-all text-muted-foreground">
          {field} = {value}
        </p>
      ))}
