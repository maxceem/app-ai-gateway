...
let appleAppAttestRoot: X509Certificate | undefined;

/**
 * Parses the Apple App Attest root on first use instead of at module
 * evaluation. This module is imported dynamically by the auth routes, and
 * keeping the parse out of the module body means an isolate that never
 * attests never pays for it at all.
 */
function appleAppAttestRootCertificate(): X509Certificate {
  appleAppAttestRoot ??= new X509Certificate(APPLE_APP_ATTEST_ROOT_PEM);
  return appleAppAttestRoot;
}
...
export async function verifyAppAttestation(input: {
  ...
  const root = appleAppAttestRootCertificate();
  if (
