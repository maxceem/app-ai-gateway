...
/**
 * Retention for the authentication event log and for spent App Attest
 * challenges, and nothing else.
 *
 * Deliberately the only scheduled writes this Worker performs: usage events are
 * accounting history, and no cron is ever allowed near them. The two sweeps get
 * their own try/catch so a failure in one still leaves the other to run.
 */
