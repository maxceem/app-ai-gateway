...
/**
 * The adapter behind an active gateway row, read without touching the vault.
 *
 * Attaching a provider to a gateway needs the gateway's *type* and nothing
 * else: it decides whether the gateway can carry this provider's traffic and
 * whether it accepts the routing configuration being stored. Nothing here
...
providerRoutes.put("/providers/:id", async (c) => {
  ...
  if (body.baseUrl !== undefined) {
    ...
    // A new origin travels with the key it is to be used with. The contract
    // says so too, so this is only reachable by a caller that bypassed it, but
    // it is the check that matters: it is what keeps the stored ciphertext from
    // ever being decrypted on the strength of a URL somebody just typed.
