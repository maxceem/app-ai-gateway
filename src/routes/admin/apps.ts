...
routes.handle("listApps", (c) =>
  listApps(managementScope(c), c.get("actor"), c.req.query("month") ?? currentMonth()));
...
routes.handle("deleteApp", (c) =>
  deleteApp(managementScope(c), c.get("actor"), c.req.param("app"), c.req.query("confirm")));
