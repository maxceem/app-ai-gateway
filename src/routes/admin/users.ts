...
const IDENTITIES_SQL = `
  WITH identities AS (
    ...
    SELECT events.user_id AS id, 'active' AS status, NULL AS attest_key_id,
     ...
     WHERE events.app_id = ?
       -- Userless traffic belongs to no user, so it synthesizes none. An
       -- application that identifies nobody has an empty user list, which is
       -- the true answer rather than one row standing for all of it.
