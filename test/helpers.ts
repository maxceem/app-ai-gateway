 ...
 * tests must carry these as `iss` and `aud` or the gateway refuses them.
 */
...
export function limitsConfig(options: SeedOptions): Record<string, unknown> {
  ...
  return {
    limits: {
      per_user: {
        requests: { per_minute: options.limits?.rpm ?? null, per_day: options.limits?.rpd ?? null },
        spending: { monthly_usd: options.budgetUsd ?? null },
      },
      per_app: {
        requests: { per_minute: options.limits?.app_rpm ?? null, per_day: options.limits?.app_rpd ?? null },
...
export async function seedApp(
  appId: string,
  options: SeedOptions & { endUser?: "app_install" } = {},
): Promise<void> {
  ...
  await database(env.DB).insert(app).values({
    ...
    config: {
      authentication: {
        ...
        end_user: options.endUser === "app_install"
          ? { source: "app_install" }
          : {
            source: "issuer",
            issuer: {
              jwks_url: issuer.jwks_url ?? "https://issuer.test/.well-known/jwks.json",
              issuer: issuer.issuer ?? TEST_ISSUER,
              audience: issuer.audience ?? TEST_AUDIENCE,
...
export async function seedServerApp(
  ...
  options: SeedOptions & {
    ...
    issuer?: Record<string, unknown>;
    /**
     * `"none"` seeds an application that identifies no end users; `"header"` is
     * the default and names {@link endUserHeader}.
     */
    endUser?: "none" | "header";
    /** The header a `"header"` source reads its user id from. */
          ...
          end_user: options.issuer === undefined
            ...
            : {
              ...
              issuer: {
                ...
                issuer: options.issuer.issuer ?? TEST_ISSUER,
                audience: options.issuer.audience ?? TEST_AUDIENCE,
