...
describe("auth event retention", () => {
  it("drops attempts past the window and leaves billing history alone", async () => {
     ...
     * instead of simply dropping them: the raw row goes, and its request,
     * dimensions and cost survive in `app_usage_rollup`. That is the difference
     * between the two tables, and this asserts it through the real cron rather
     * than by calling the retention functions directly.
     */
    await expect(
    ...
    await expect(
      env.DB.prepare(
        `SELECT COALESCE(SUM(requests), 0) AS requests, model, provider_type, status
           FROM app_usage_rollup WHERE app_id = 'prune-app'`,
      ).first<{ requests: number; model: string; provider_type: string; status: string }>(),
    ).resolves.toEqual({
      requests: 1,
      model: "gpt-5.6-sol",
