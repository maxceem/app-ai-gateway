...
describe("provider self-reports", () => {
  ...
  it("reads the cost, the serving host, and the served model out of one response", () => {
    const seen = observeResponse(
      wholeBody(completion({ cost: 0.00025 }, {
        openrouter_metadata: {
          endpoints: {
            total: 2,
            available: [
...
/**
 * A response body is observed through two fixed windows rather than buffered
 * whole, so a batch of embeddings or a long stream cannot decide how much
 * memory an isolate holds. Every shape priced here reports at one end of the
 * body or the other, which is what makes the middle safe to drop.
 */
describe("large response bodies", () => {
  const SIX_MB = 6 * 1024 * 1024;

  /** What the observer makes of one body, through the pipe the proxy uses. */
  async function observe(text: string): Promise<ObservedBody> {
    const { stream, observed } = observeUpstreamBody(new Response(text).body!);
    await new Response(stream).arrayBuffer();
    return observed;
  }

  /** An embeddings response of at least `bytes`, with its usage at the end. */
  function embeddingsBody(bytes: number): string {
    const vector = `[${"0.0123456789,".repeat(999)}0.0123456789]`;
    const rows = Math.ceil(bytes / vector.length);
    const data = Array.from(
      { length: rows },
      (_unused, index) => `{"object":"embedding","index":${index},"embedding":${vector}}`,
    ).join(",");
    return `{"object":"list","data":[${data}],"model":"text-embedding-3-small",`
      + `"usage":{"prompt_tokens":120000,"total_tokens":120000}}`;
  }

  /** An Anthropic stream of at least `bytes`, reporting at both of its ends. */
  function anthropicStream(bytes: number): string {
    ...
    const delta = `event: content_block_delta\ndata: ${JSON.stringify({
      ...
      index: 0,
      delta: { type: "text_delta", text: "lorem ipsum ".repeat(80) },
    })}\n\n`;
    ...
    return start + delta.repeat(Math.ceil(bytes / delta.length)) + end;
  }

  it("prices a 6 MB embeddings response from the usage at its end", async () => {
  ...
  it("reads no usage from a 6 MB body that reports none", async () => {
    const observed = await observe(`{"object":"list","data":"${"x".repeat(SIX_MB)}"}`);
    expect(observed.truncated).toBe(true);
  ...
  it("reads the usage object out of a tail full of braces and quotes", async () => {
    // The tail is scanned with a bracket counter rather than parsed, so a
    // string holding a closing brace or an escaped quote must not end it early.
    const usage = String.raw`{"note":"a } brace and an \" escaped quote","prompt_tokens":42,`
      + `"completion_tokens":7}`;
  ...
  it(
    "gives up on a tail whose only usage key carries no object",
    () => {
      // The window can open exactly on the key, and a value that is not an
  ...
  it("leaves a body that fits the windows untouched", async () => {
    const text = JSON.stringify({
      usage: { prompt_tokens: 30, completion_tokens: 5 },
      filler: "x".repeat(600 * 1024),
    });
    const observed = await observe(text);
    // Larger than the head window and rebuilt byte for byte from the overlap,
  ...
  it("never retains more than the head and tail windows", () => {
    const windows = bodyWindows();
    const encoder = new TextEncoder();
    const chunks = 200;
    for (let index = 0; index < chunks; index += 1) {
      windows.push(encoder.encode(`${index}`.padStart(8, "0").repeat(8 * 1024)));
      expect(windows.retainedBytes())
        .toBeLessThanOrEqual(OBSERVER_HEAD_BYTES + OBSERVER_TAIL_BYTES);
    }
    const read = windows.read();
    expect(read.totalBytes).toBe(chunks * 64 * 1024);
    expect(read.truncated).toBe(true);
    expect(read.head.length).toBe(OBSERVER_HEAD_BYTES);
    expect(read.tail.length).toBe(OBSERVER_TAIL_BYTES);
    // The windows are the two ends of the body, not any two slices of it.
    expect(read.head.startsWith("00000000")).toBe(true);
    expect(read.tail.endsWith(`${chunks - 1}`.padStart(8, "0"))).toBe(true);
  });
});
