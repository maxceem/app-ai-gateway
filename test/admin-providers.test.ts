...
describe("operator-configurable base URL", () => {
  ...
  it("moves the origin when the key comes with it, probing the new key there", async () => {
    const urls = stubProbe();
    const created = await call("POST", "/v1/admin/providers", {
      type: "openai",
      name: "Custom",
    ...
    // Proven with the key that was just supplied, at the origin it is moving to.
  ...
  it("clears the origin without a key and without a probe", async () => {
    ...
    // Back to an origin the key already talks to, so there is nothing to prove.
    expect(cleared.body.validated).toBeNull();
    expect(urls).toEqual([]);
    expect(decrypt).not.toHaveBeenCalled();
    clearProviderCaches();
