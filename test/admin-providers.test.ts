...
describe("admin provider instances", () => {
  it("creates a direct provider with its default slug and never returns the key", async () => {
    ...
    // A write stores what it is given. Checking the key is a separate request
    // the operator makes when they want it, so nothing was called here.
    expect(urls).toEqual([]);
  ...
  it("stores a credential the provider would refuse, without asking the provider", async () => {
    const urls = stubProbe("rejected");
    const stored = await call("POST", "/v1/admin/providers", {
      type: "anthropic",
      name: "Bad key",
      secret: "bad-key",
    });

    // The operator may know something the probe does not — a key not live yet,
    // a provider having an outage — so a write never judges the credential.
    // `POST /providers/test` is the only thing that calls out, on request.
    expect(stored.status, stored.text).toBe(201);
    expect(urls).toEqual([]);
    expect((await call("GET", "/v1/admin/providers")).body.providers).toHaveLength(1);
  });
...
describe("admin provider gateway API", () => {
  it("creates, lists, renames, and encrypts a reusable gateway token", async () => {
    const urls = stubProbe();
    const gateway = await createGateway();
    // Stored as given: the token is checked only by the dry run.
    expect(urls).toEqual([]);
  ...
  it("stores a connection the gateway would refuse, on create and on rotate", async () => {
    const urls = stubProbe("rejected");
    // A Cloudflare AI Gateway answers 401 for a wrong token and for one that is
    // not finished being set up, which is the clearest reason a write must not
    // judge a token: the operator adds the connection and fixes the rest.
    const created = await call("POST", "/v1/admin/provider-gateways", {
      type: "cf_aig",
  ...
  it("attaches providers individually, and never stores per-row secrets", async () => {
    const urls = stubProbe();
    ...
    // Attaching needs the gateway's type and nothing else, so the shared token
    // is neither spent nor unsealed.
    expect(urls).toEqual([]);
...
describe("gateway routing configuration", () => {
  ...
  it("creates a Vercel gateway from a name and a token alone", async () => {
    ...
    // Neither the create nor the rotation called Vercel.
    expect(urls).toEqual([]);
  });

  it("stores a Vercel connection the gateway would refuse", async () => {
    const urls = stubProbe("rejected");
    // Exactly as a Cloudflare connection is: a gateway write stores the token
    // it is handed, whatever the gateway would say about it.
    const created = await call("POST", "/v1/admin/provider-gateways", {
      type: "vercel",
      name: "Team gateway",
      token: "vck_not_a_key",
...
describe("operator-configurable base URL", () => {
  it("stores the canonical origin and proxies through it", async () => {
    const urls = stubProbe();
    const created = await call("POST", "/v1/admin/providers", {
      type: "openai",
      name: "Self-hosted",
    ...
    // Neither OpenAI's origin nor the operator's was called: nothing is probed
    // on a write.
    expect(urls).toEqual([]);
  ...
  it("refuses a base URL alongside a gateway, on create and on update", async () => {
    ...
    // The contract asks for the key before it looks at the row at all, so this
  ...
  it("refuses a move that does not re-supply the key, and never decrypts to probe", async () => {
    const urls = stubProbe();
    const created = await call("POST", "/v1/admin/providers", {
      type: "openai",
      name: "Custom",
    ...
    // The whole point: no probe went anywhere, and the stored key stayed sealed.
    expect(urls).toEqual([]);
    expect(decrypt).not.toHaveBeenCalled();
    // And nothing was written.
    const listed = await call("GET", "/v1/admin/providers");
    expect((listed.body.providers as ProviderSummary[])[0]?.baseUrl)
      .toBe("https://first.example.com/v1/");
  });

  it("moves the origin when the key comes with it", async () => {
    const urls = stubProbe();
    const created = await call("POST", "/v1/admin/providers", {
      type: "openai",
      name: "Custom",
    ...
    expect(urls).toEqual([]);
    // Both were stored: the hint is the new key's.
    expect(moved.body.provider.secretHint).toBe("alue");
    expect(decrypt).not.toHaveBeenCalled();
    clearProviderCaches();
    const resolved = await resolveProvider(env, TEST_ORGANIZATION_ID, "openai");
    expect(resolved?.baseUrl).toBe("https://second.example.com/v1/");
    expect(resolved?.secret).toBe("sk-second-origin-value");
  });

  it("clears the origin without a key and without a probe", async () => {
    const urls = stubProbe();
    const created = await call("POST", "/v1/admin/providers", {
      type: "openai",
      name: "Custom",
  ...
  it("refuses a dry run that pairs a stored gateway with an origin of its own", async () => {
    const urls = stubProbe();
    const gateway = await createGateway();
    urls.length = 0;
    ...
    // Otherwise the dry run would be the same read-back the update refuses:
    // the gateway's stored token, probed at an origin the operator just typed.
    expect(refused.status, refused.text).toBe(400);
    expect(refused.body.error.message).toMatch(/gateway owns the upstream origin/u);
    expect(urls).toEqual([]);
  });
});

describe("provider probe coverage", () => {
  it("probes every provider type at its own documented path", async () => {
    ...
    for (const type of PROVIDER_TYPES) {
      ...
      expect([type, tested.status]).toEqual([type, 200]);
    }
    // A provider with no probe of its own contributes no request here: it
    // reports that nothing was checked rather than guessing at a URL.
  ...
  it("says nothing was checked for a provider with no probe of its own", async () => {
    ...
    for (const type of ["perplexity", "fireworks", "huggingface"] as const) {
      ...
      expect([type, tested.status, tested.body]).toEqual([
        type,
        200,
        { validated: false, reason: "no_probe" },
      ]);
    }
