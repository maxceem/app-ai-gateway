...
async function signingFixture(kid: string): Promise<{
  ...
  const token = async (claims: Record<string, unknown> = {}, lifetime = 300): Promise<string> => {
    ...
    if (claims.aud === undefined) jwt.setAudience(AUDIENCE);
...
describe("issuer JWT verification", () => {
  ...
  it("matches audience, scope, and nested array requirements uniformly", async () => {
    ...
    const config: IssuerAuthConfig = {
      ...
      // clear the audience gate and only the requirement separates them.
      audience: ["mobile"],
      required_claims: [
        { path: "aud", contains: "app-ai-gateway" },
        { path: "scope", contains: "ai.invoke" },
        { path: "claims.entitlements", contains: "pro" },
      ],
    };
    const valid = await fixture.token({
      aud: ["mobile", "app-ai-gateway"],
  ...
  it("accepts only Firebase-shaped tokens from the configured project with the pro entitlement", async () => {
    ...
    // The shared Firebase key set signs this one too, so the signature holds.
    // Only `iss` separates the operator's project from the attacker's.
  ...
  it("refuses a correctly signed token minted for another tenant", async () => {
    // The whole point of the two claims: a shared key set — Firebase, Sign in
    // with Apple, Google Sign-In — verifies an attacker's own project's tokens
    // just as happily as the operator's.
    const fixture = await signingFixture("key-tenant");
    vi.spyOn(globalThis, "fetch")
      .mockImplementation(() => Promise.resolve(Response.json({ keys: [fixture.publicJwk] })));

    await expect(
      verifyIssuerToken(await fixture.token({ iss: "https://attacker.example/" }), baseConfig),
    ...
    await expect(
      verifyIssuerToken(await fixture.token({ aud: "attacker-project" }), baseConfig),
    ).rejects.toMatchObject({
      status: 403,
      code: "issuer_token_rejected",
  ...
  it("accepts any listed issuer and audience when several are configured", async () => {
    // A list is what makes a migration between two projects a config edit
    ...
    await expect(verifyIssuerToken(await fixture.token(), config)).resolves.toMatchObject({
      userId: "issuer-user",
    });
    const migrated = await fixture.token({ iss: "https://old-issuer.test/", aud: "old-audience" });
    await expect(verifyIssuerToken(migrated, config)).resolves.toMatchObject({
      userId: "issuer-user",
    });
  });
  ...
  it("spends one JWKS fetch on unknown kids per cooldown, and still sees a rotation", async () => {
    // The fake clock is what makes the cooldown observable; `Date.now` is all
    // the gateway reads to decide whether an unknown kid may cost a round trip.
    vi.useFakeTimers();
    const rotated = await signingFixture("rotated-key");
    // A fresh Response per call: a body can only be read once, and reusing one
    // would make a refetch fail to parse rather than come back keyless —
    // which is a different rejection entirely.
    let served: JWK[] = [];
    const fetchSpy = vi.spyOn(globalThis, "fetch")
      .mockImplementation(() => Promise.resolve(Response.json({ keys: served })));

    // The cache is cold, so this one fetch is unavoidable — and the kid is
    // absent from keys read a moment ago, so no second round trip can help.
    await expect(verifyIssuerToken(await rotated.token(), baseConfig)).rejects.toMatchObject({
      code: "issuer_token_rejected",
      reason: "unknown_kid",
    });
    expect(fetchSpy).toHaveBeenCalledTimes(1);

    // A caller sending random kids must not be able to buy a round trip each.
    await expect(verifyIssuerToken(await rotated.token(), baseConfig)).rejects.toMatchObject({
      code: "issuer_token_rejected",
      reason: "unknown_kid",
    });
    ...
    // A real rotation is still picked up: past the cooldown, and well inside
    // the cache TTL, the unknown kid earns exactly one more fetch.
    served = [rotated.publicJwk];
    vi.advanceTimersByTime(61_000);
    await expect(verifyIssuerToken(await rotated.token(), baseConfig)).resolves.toMatchObject({
      userId: "issuer-user",
    });
    expect(fetchSpy).toHaveBeenCalledTimes(2);
  });

  it("shares one JWKS fetch across concurrent verifications", async () => {
    ...
    await expect(
      Promise.all([
        verifyIssuerToken(token, baseConfig),
        verifyIssuerToken(token, baseConfig),
        verifyIssuerToken(token, baseConfig),
      ]),
  ...
  it("gives up on a JWKS endpoint that accepts the connection and never answers", async () => {
    const fixture = await signingFixture("hanging-key");
    // Real time on purpose: workerd's `AbortSignal.timeout` runs on the
    // runtime's own clock, which vitest's fake timers do not drive.
    vi.spyOn(globalThis, "fetch").mockImplementation((_input, init) => {
      const signal = init?.signal;
      return new Promise<Response>((_resolve, failFetch) => {
        signal?.addEventListener("abort", () => failFetch(signal?.reason));
      });
    });
    ...
    // Answered by the deadline rather than by never returning at all.
