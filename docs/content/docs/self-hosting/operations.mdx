...
After upgrading to the release that binds direct provider keys to their type
and destination, re-enter or rotate each direct provider key once. Ciphertext
written before that change is deliberately rejected because it lacks the new
...
A single cron trigger runs every minute. At 03:17 UTC it uses that invocation
for nightly maintenance instead of spend recovery and keeps the database bounded:
...
[`MAINTENANCE_QUERY_BUDGET`](/self-hosting/configuration) lets one run do more.
...
Saved spend changes are sent to the app's limit counters immediately. That
work is bounded for a large reprice: `reconciled_users` is the number of user
counters acknowledged during this request, and the remaining saved updates
stay pending. The same cron trigger retries pending updates every minute except
the 03:17 UTC maintenance minute. Replays and
out-of-order deliveries are safe, so a transient counter failure cannot add the
same spend twice or replace a newer total with an older one.
...
Disabling or deleting a provider can take up to 60 seconds to reach another
warm Worker isolate because provider rows are cached; for an immediate
emergency stop, revoke the key in the provider account too.
...
The rows the gateway caches — an app, its providers, its keys and its account
row — are read through a D1 read-replication session. If you turn on read
replication for the database in the Cloudflare dashboard (**D1** → your
database → **Settings**), a Worker running far from the database's primary
answers those cold reads from the nearest replica instead of crossing a region
for each one; without it nothing changes, and there is nothing to configure in
the gateway either way. Those caches already tolerate up to a minute of
staleness, so a replica running slightly behind costs them nothing. Writes, and
every read behind the console and the CLI, still go to the primary.
