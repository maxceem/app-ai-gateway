...
the billing service's default plan. There are no exceptions: an
...
The allowance is not a per-user limit and does not cap what an organization may
grant its own users. An organization sets those separately, per app, and they
