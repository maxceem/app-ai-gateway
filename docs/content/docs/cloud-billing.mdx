...
It is a whole, non-negative count of requests the organization may dispatch
...
organization unlimited. The value may be written as a number or as a JSON
...
The period follows the organization rather than the calendar:

- The default Free plan starts a period at the exact UTC date and time the
  organization was created, then repeats monthly from that anniversary.
...
schedule. When an organization returns to Free, it returns to the original
organization-anniversary schedule and to the count already recorded in that
Free period.
